...
class CameraFinder:
    ...
    def __init__(self, realsense_enable=False, verbose=False):
        ...
        # uvc (optional - not needed for Isaac Sim cameras)
        try:
            reload_uvc_driver()
            import uvc
            ...
            logger_mp.warning("[CameraFinder] 'uvc' module not found, UVC camera support disabled (OK for Isaac Sim mode).")
            self.uvc_devices = []
            self.uid_map = {}
        # all video devices
...
class IsaacSimCamera(BaseCamera):
    def __init__(self, cam_topic, img_shape, fps,
        ...
        Args:
            ...
            binocular: if True, concatenate left+right for binocular vision
        """
        super().__init__(cam_topic, img_shape, fps, enable_zmq, zmq_port, enable_webrtc, webrtc_port, webrtc_codec)
